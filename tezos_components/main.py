...
class FA2_mint(FA2_core):
    ...
    def unmint(self, params):
        ...
        if self.config.non_fungible:
            sp.verify(params.amount == 1, message = "NFT-asset: amount <> 1")
        user = self.ledger_key.make(params.address, params.token_id)
        sp.verify(self.data.ledger.contains(user), message="User doesn't own this token")
        sp.verify(self.data.ledger[user].balance >= params.amount, message="User doesn't have sufficient amount of that tokens")
        sp.if self.data.ledger[user].balance > params.amount:
            self.data.ledger[user].balance = sp.is_nat(self.data.ledger[user].balance - params.amount).open_some(message = "balance is smaller then requested amount")         
        sp.else:
...
class MultiSigMintery(sp.Contract):
    def __init__(self, params):
        self.init(
            ...
            pks = params.pks,
            nft_contract=sp.address("tz1SW56fUwARi14KJcn6bNtErFK8ocE2yq3m"),
            requested_unmints=sp.big_map(
                tkey=sp.TRecord(
                    address=sp.TAddress, 
                    token_id=sp.TNat, 
                    amount=sp.TNat
                ),
                tvalue=sp.TString # target_address
            )
        )

    @sp.entry_point
    def request_unmint(self, params):
        self.data.requested_unmints[sp.record(
            address=sp.sender,
    ...
    @sp.entry_point
    def unmint_token(self, params):
        # Verify signatures
        sp.verify(params.exp_block >= sp.level, "That message expired")
        message_packed = sp.pack(sp.pair(
            params.unmint_params,
            params.exp_block
        ))
        self.verify_signatures(sp.record(payload=message_packed, signatures=params.signatures))
        
        # Do unmint
        sp.verify(self.data.requested_unmints.contains(params.unmint_params), "Unmint execution doesn't match any unmint orders")
        ...
        sp.transfer(params.unmint_params, sp.tez(0), contract)
        del self.data.requested_unmints[params.unmint_params]
    ...
    def set_minter(self, params):
        ...
        # Set minter
        self.data.nft_contract = params.nft_contract
...
############################################
        ...
        scenario.h2("test set_minter")
        alice_sign = sp.make_signature(
            alice.secret_key, 
            sp.pack(sp.pair(
                c1.address,
                1003)), 
        ...
        m.set_minter(sp.record(
            signatures=[
                alice_sign,
                bob_sign
            ],
            exp_block=1003,
            nft_contract=c1.address)
        ).run(level=1000)
        m.set_minter(sp.record(
            signatures=[
                alice_sign
        ...
        scenario.h2("test request unmint")
        m.request_unmint(sp.record(
                amount=sp.nat(1),
                token_id=sp.nat(0),
                target=sp.string("0x123456")
        ...
        m.unmint_token(sp.record(
            signatures=[
                alice_sign,
                bob_sign
            ],
            exp_block=1003,
            unmint_params = sp.record(
                address=alice.address,
                amount=sp.nat(1),
                token_id=sp.nat(0)
            ))
        ...
        #         token_id=sp.nat(2)
        #     )))
        # m.update_signers(sp.record(pks=[
        #         alice.public_key,
        #         bob.public_key,
        #     ]))
